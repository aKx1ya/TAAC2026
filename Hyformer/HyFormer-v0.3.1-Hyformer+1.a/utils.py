...
class EarlyStopping:
    ...
    def __init__(
        ...
        self.rank: int = rank  # Only rank 0 performs filesystem I/O
        if self.label != "":
            self.label += " "
    ...
    def save_checkpoint(self, score: float, model: nn.Module) -> None:
        ...
        In DDP mode, only rank 0 writes to disk; other ranks update the
        in-memory score tracking but skip I/O.
