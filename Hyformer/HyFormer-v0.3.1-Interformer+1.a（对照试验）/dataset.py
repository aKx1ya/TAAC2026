...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        #
        # Scans the (already filtered) training Row Groups to count how
        # often each item_id (fid=16) appears, then builds an in-memory
        # lookup table.  Validation datasets receive the SAME freq map
        # via a shared reference (see get_pcvr_data).  The item_dense
        # schema definition is printed to the log so the user can capture
        # it and update their local schema.json for future runs.
        # ═══════════════════════════════════════════════════════════════
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._freq_map_shared: bool = False  # set True when map injected externally
    ...
    def _build_item_freq_map(self) -> None:
        ...
        Only called when ``item_dense`` is defined in schema.json and the
        dataset covers the training split (``row_group_range`` excludes the
        validation tail).  Validation datasets share the map via
        ``get_pcvr_data`` (see the ``share_item_freq_map`` logic below).

        The computed schema is printed to the log so the user can verify
        and update their local ``schema.json``.
        """

        # Locate the item_id column in parquet.
        ...
        # Scan only the Row Groups assigned to this dataset split
        # (already filtered by row_group_range).
        # fid=16 has dim=1 — it is a scalar int64 column, not a list.
        # rg.column() returns a ChunkedArray; fill_null(0).to_numpy()
        # works for both Array and ChunkedArray.
        ...
        for file_path, rg_idx, _ in self._rg_list:
            ...
            # Keep only positive item_ids (0 = padding / missing).
        ...
        # ── Print schema info to log (user can capture this) ─────────
        logging.info(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            "  ─────────────────────────────────────────────────────────\n"
            "  Copy the schema entry above into your local schema.json.\n"
            "  Future runs will pick it up automatically — the freq map\n"
            "  is always computed online, no manual step needed.\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            f"{scanned_rows:,}",
        ...
        # Record the offset of fid=16 in the flat item_int buffer so
        # _convert_batch can extract item_ids for freq lookup.
        try:
            self._item_id_offset, _ = self.item_int_schema.get_offset_length(16)
        except KeyError:
            self._item_id_offset = None

        # Record the offset of fid=200 (log-freq) in the item_dense
        # buffer so _convert_batch can write to the correct slot when
        # item_dense has multiple features (e.g. Step 1b CTR).
        try:
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- item_dense: online lookup from item freq map (Step 1a) ----
        item_dense = self._buf_item_dense[:B]
        item_dense[:] = 0.0
        if self._item_freq_map is not None and self._item_id_offset is not None:
            # item_int is already np.int64 — no .astype() copy needed.
            item_ids = item_int[:, self._item_id_offset]
            col = self._item_dense_freq_offset
            for i in range(B):
                item_id = int(item_ids[i])
                if item_id > 0 and item_id in self._item_freq_map:
                    item_dense[i, col] = self._item_freq_map[item_id]
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    # The validation dataset must use training statistics to avoid label
    # leakage.  We copy the in-memory map and offsets from the
    # already-initialised training dataset.
