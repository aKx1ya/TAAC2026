...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 weighted_fid_config: Optional[Dict[int, Tuple[int, int]]] = None) -> None:
        super().__init__()
            ...
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        weights = F.softmax(torch.log1p(dense_vals.clamp(min=0)), dim=-1)
                        mask = (vals != 0).float()
            ...
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        vals = int_feats[:, offset:offset + length].long()
                        emb_all = emb_layer(vals)          # (B, length, emb_dim)
                        # log1p compression → softmax → mask padding positions
                        weights = F.softmax(torch.log1p(dense_vals.clamp(min=0)), dim=-1)  # (B, length)
