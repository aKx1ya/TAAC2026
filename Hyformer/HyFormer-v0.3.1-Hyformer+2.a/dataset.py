...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        # ── Step 1a: Online item-frequency computation for item_dense ──
        if self._item_dense_enabled and self._rg_list and not self._freq_map_shared:
            self._build_item_freq_map()

        # ── Step 1b: resolve CTR offset & compute Bayesian smoothed CTR ──
        try:
            self._item_dense_ctr_offset, _ = self.item_dense_schema.get_offset_length(201)
        except KeyError:
            self._item_dense_ctr_offset = 0

        if self._item_dense_enabled and self._rg_list and not self._ctr_map_shared:
            self._build_item_ctr_map()
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._item_freq_map: Optional[Dict[int, float]] = None
        self._item_dense_enabled: bool = len(self._item_dense_cols) > 0
        self._item_id_offset: Optional[int] = None
        self._item_dense_freq_offset: int = 0
        self._freq_map_shared: bool = False
        self._item_ctr_map: Optional[Dict[int, float]] = None
        self._item_dense_ctr_offset: int = 0
        self._ctr_map_shared: bool = False
    ...
    # ── Step 1a: Item frequency map ──

    def share_item_freq_map(
        self,
        freq_map: Dict[int, float],
        item_id_offset: Optional[int],
        item_dense_freq_offset: int,
    ) -> None:
        """Inject an externally-computed item freq map (used by validation
        dataset to reuse training statistics, preventing label leakage)."""
        self._item_freq_map = freq_map
        self._item_id_offset = item_id_offset
        self._item_dense_freq_offset = item_dense_freq_offset
        self._freq_map_shared = True

    def _build_item_freq_map(self) -> None:
        """Scan training Row Groups to count item_id (fid=16) occurrences.

        Builds ``self._item_freq_map``: a dict mapping ``item_id → log1p(count)``.
        Only called on the training split.  Validation datasets share the map
        via ``get_pcvr_data`` (see ``share_item_freq_map`` below).
        """

        item_col_name = "item_int_feats_16"
        if item_col_name not in self._col_idx:
            logging.warning(
                "Step 1a: column '%s' not found in parquet; "
                "item_dense will be all zeros", item_col_name
            )
            self._item_freq_map = {}
            return

        item_ci = self._col_idx[item_col_name]

        freq: dict[int, int] = defaultdict(int)
        scanned_rows = 0
        for file_path, rg_idx, _ in self._rg_list:
            pf = pq.ParquetFile(file_path)
            rg = pf.read_row_group(rg_idx, columns=[item_col_name])
            col = rg.column(item_col_name)
            all_values = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
            scanned_rows += col.length()
            valid = all_values[all_values > 0]
            if len(valid) == 0:
                continue
            unique_ids, counts = np.unique(valid, return_counts=True)
            for item_id, count in zip(unique_ids, counts):
                freq[int(item_id)] += int(count)
        ...
        # ── Build numpy array for vectorised lookup (avoid per-row dict access) ──
        max_item_id = max(freq.keys()) if freq else 0
        self._freq_array = np.zeros(max_item_id + 1, dtype=np.float32)
        for item_id, count in freq.items():
            self._freq_array[item_id] = float(np.log1p(count))

        logging.info(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "  Step 1a: item_dense (log1p item frequency) computed online\n"
            "  ─────────────────────────────────────────────────────────\n"
            "  Scanned rows  : %s\n"
            "  Unique items  : %s\n"
            "  item_dense dim: %s\n"
            "  Schema entry  : \"item_dense\": %s\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            f"{scanned_rows:,}",
            f"{len(self._item_freq_map):,}",
            self.item_dense_schema.total_dim,
            json.dumps(self._item_dense_cols),
        )

        try:
            self._item_id_offset, _ = self.item_int_schema.get_offset_length(16)
        except KeyError:
            self._item_id_offset = None

        try:
            self._item_dense_freq_offset, _ = self.item_dense_schema.get_offset_length(200)
        except KeyError:
            self._item_dense_freq_offset = 0

    # ── Step 1b: Bayesian smoothed CTR map ──

    def share_item_ctr_map(
        self,
        ctr_map: Dict[int, float],
        item_id_offset: Optional[int],
        item_dense_ctr_offset: int,
    ) -> None:
        """Inject an externally-computed Bayesian smoothed CTR map (used by
        validation dataset to reuse training statistics, preventing label
        leakage)."""
        self._item_ctr_map = ctr_map
        self._item_id_offset = item_id_offset
        self._item_dense_ctr_offset = item_dense_ctr_offset
        self._ctr_map_shared = True

    def _build_item_ctr_map(self) -> None:
        """Scan training Row Groups to compute Bayesian smoothed CTR per item.

        For each item_id (fid=16), counts impressions and clicks (label==1),
        then smooths:

            smooth_ctr = (clicks + alpha * global_ctr) / (impressions + alpha)

        where global_ctr is the overall click rate across all items.
        Only called on the training split. Validation datasets share the map
        via ``get_pcvr_data`` (see ``share_item_ctr_map``).
        """
        alpha = 100.0  # Bayesian smoothing strength

        item_col_name = "item_int_feats_16"
        label_col_name = "label_type"

        if item_col_name not in self._col_idx:
            logging.warning(
                "Step 1b: column '%s' not found in parquet; "
                "item_dense CTR will be all zeros", item_col_name
            )
            self._item_ctr_map = {}
            return

        item_ci = self._col_idx[item_col_name]
        label_ci = self._col_idx.get(label_col_name)
        ...
        for file_path, rg_idx, _ in self._rg_list:
            pf = pq.ParquetFile(file_path)
            rg = pf.read_row_group(rg_idx, columns=[item_col_name, label_col_name])

            col_item = rg.column(item_col_name)
            all_values = col_item.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
            scanned_rows += col_item.length()

            col_label = rg.column(label_col_name)
            all_labels = col_label.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)

            valid_mask = all_values > 0
            valid_items = all_values[valid_mask]
            valid_labels = all_labels[valid_mask]

            if len(valid_items) == 0:
                continue

            unique_ids, inverse, counts = np.unique(valid_items, return_inverse=True, return_counts=True)
            for item_id, count in zip(unique_ids, counts):
                impressions[int(item_id)] += int(count)

            click_mask = valid_labels == 1
            if click_mask.any():
                click_items = valid_items[click_mask]
                unique_click_ids, click_counts = np.unique(click_items, return_counts=True)
                for item_id, count in zip(unique_click_ids, click_counts):
                    clicks[int(item_id)] += int(count)

        if not impressions:
            logging.warning("Step 1b: no item_id data found; item_dense CTR will be all zeros")
            self._item_ctr_map = {}
            return

        total_impressions = sum(impressions.values())
        total_clicks = sum(clicks.values())
        global_ctr = total_clicks / max(total_impressions, 1)

        # Bayesian smoothed CTR + numpy array for vectorised lookup
        self._item_ctr_map = {}
        max_item_id = 0
        for item_id, imp in impressions.items():
            clk = clicks.get(item_id, 0)
            smooth_ctr = (clk + alpha * global_ctr) / (imp + alpha)
            self._item_ctr_map[item_id] = float(smooth_ctr)
            if item_id > max_item_id:
                max_item_id = item_id

        self._ctr_array = np.zeros(max_item_id + 1, dtype=np.float32)
        for item_id, imp in impressions.items():
            clk = clicks.get(item_id, 0)
            self._ctr_array[item_id] = float((clk + alpha * global_ctr) / (imp + alpha))

        logging.info(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "  Step 1b: item_dense (Bayesian smoothed CTR) computed online\n"
            "  ─────────────────────────────────────────────────────────\n"
            "  Scanned rows      : %s\n"
            "  Unique items      : %s\n"
            "  Total impressions : %s\n"
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- item_dense: vectorised lookup from freq array (Step 1a) ----
        item_dense = self._buf_item_dense[:B]
        item_dense[:] = 0.0
        if hasattr(self, '_freq_array') and self._item_id_offset is not None:
            item_ids = item_int[:, self._item_id_offset]
            col = self._item_dense_freq_offset
            valid_mask = (item_ids > 0) & (item_ids < len(self._freq_array))
            item_dense[valid_mask, col] = self._freq_array[item_ids[valid_mask]]

        # ---- item_dense: vectorised lookup from CTR array (Step 1b) ----
        if hasattr(self, '_ctr_array') and self._item_id_offset is not None:
            item_ids = item_int[:, self._item_id_offset]
            col = self._item_dense_ctr_offset
            valid_mask = (item_ids > 0) & (item_ids < len(self._ctr_array))
            item_dense[valid_mask, col] = self._ctr_array[item_ids[valid_mask]]
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    # ── Step 1a: share the train-computed freq map with validation ──
    _shared_freq_map = getattr(train_dataset, '_item_freq_map', None)
    _shared_item_id_offset = getattr(train_dataset, '_item_id_offset', None)
    _shared_dense_freq_offset = getattr(train_dataset, '_item_dense_freq_offset', 0)
    # ── Step 1b: share the train-computed CTR map with validation ──
    _shared_ctr_map = getattr(train_dataset, '_item_ctr_map', None)
    _shared_dense_ctr_offset = getattr(train_dataset, '_item_dense_ctr_offset', 0)
    ...
    # Step 1a: inject training-set freq map into validation dataset.
    if _shared_freq_map is not None:
        valid_dataset.share_item_freq_map(
            _shared_freq_map, _shared_item_id_offset, _shared_dense_freq_offset,
        )
    # Step 1b: inject training-set CTR map into validation dataset.
    if _shared_ctr_map is not None:
        valid_dataset.share_item_ctr_map(
            _shared_ctr_map, _shared_item_id_offset, _shared_dense_ctr_offset,
        )
