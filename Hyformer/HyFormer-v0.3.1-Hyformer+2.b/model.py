...
# ═══════════════════════════════════════════════════════════════════════════════
# Step 2.b ― Hierarchical KV Fusion (Pyramid-style coarse→fine attention)
# ═══════════════════════════════════════════════════════════════════════════════


class HierarchicalKVFusion(nn.Module):
    """Pyramid-style hierarchical fusion for nested Aligned-KV feature pairs.

    Processes a chain of 5 (int, dense) aligned feature pairs that form a
    coarse-to-fine granularity hierarchy::

        fid 62 (coarsest,  L=6)   → vocab=11
        fid 63 (mid,       L=19)  → vocab=49
        fid 64 (mid-fine,  L=26)  → vocab=51
        fid 65 (fine,      L=111) → vocab=425
        fid 66 (finest,    L=150) → vocab=1403

    At each level:

    - **Level 0 (coarsest):** KV-weighted softmax pooling (same as Step 2.a).
    - **Level l > 0:** Cross-level attention where the coarser context
      vector acts as a **query** over the current level's embeddings (key).
      The attention score is added to the dense-value bias (log1p / T),
      producing a joint weighting that respects both *semantic relevance*
      (attention) and *statistical confidence* (dense values).

    The context vector is iteratively updated at each level via a small
    fusion MLP: ``ctx ← FusionMLP([ctx, level_out])``, allowing coarser
    signals to progressively guide finer-grained aggregation.

    Parameters
    ----------
    emb_dim : int
        Dimension of per-feature embedding vectors.
    num_levels : int
        Number of hierarchy levels (default=5 for fids 62→66).
    temperature : float
        Temperature for the dense-value softmax (shared with Step 2.a).
    """
    ...
    def __init__(
        ...
        # Cross-level attention projections
        #   query: coarse context → attention query for current fine level
        #   key:   current level's token embeddings
        self.cross_queries = nn.ModuleList([
            nn.Linear(emb_dim, emb_dim) for _ in range(num_levels - 1)
        ])
        self.cross_keys = nn.ModuleList([
            nn.Linear(emb_dim, emb_dim) for _ in range(num_levels - 1)
        ])

        # Context fusion: [coarse_ctx; level_output] → new coarse context
        self.ctx_fusions = nn.ModuleList([
            nn.Sequential(
        ...
        self.level_norms = nn.ModuleList([
            nn.LayerNorm(emb_dim) for _ in range(num_levels)
        ])

    def forward(
        self,
        int_data_list: "list[torch.Tensor]",
        dense_data_list: "list[torch.Tensor]",
        emb_layers: "list[Optional[nn.Embedding]]",
    ) -> "list[torch.Tensor]":
        """Execute the hierarchical fusion over one chain.

        Parameters
        ----------
        int_data_list : list of Tensor
            ``[ (B, L_0), ..., (B, L_{N-1}) ]`` – raw integer feature values.
        dense_data_list : list of Tensor
            ``[ (B, L_0), ..., (B, L_{N-1}) ]`` – corresponding float values.
        emb_layers : list of Embedding or None
            Per-level embedding tables (``None`` when filtered by
            ``emb_skip_threshold``; in that case a zero vector is emitted).

        Returns
        -------
        list of Tensor
            ``[ (B, emb_dim), ... ]`` – one context-aware embedding per level.
        """
        outputs: list[torch.Tensor] = []
        ctx: Optional[torch.Tensor] = None  # coarse context propagated downward

        for level in range(self.num_levels):
            int_vals = int_data_list[level]      # (B, L)
            dense_vals = dense_data_list[level]  # (B, L)
            emb_layer = emb_layers[level]
            B = int_vals.shape[0]

            mask = (int_vals != 0).float()  # (B, L) – 0 = padding

            if emb_layer is None:
                # Feature skipped by emb_skip_threshold → zero vector
                level_out = int_vals.new_zeros(B, self.emb_dim)
            else:
                emb = emb_layer(int_vals)  # (B, L, emb_dim)

                if level == 0:
                    # ── Coarsest level: KV-weighted pooling (like Step 2.a) ──
                    log_vals = torch.log1p(dense_vals.clamp(min=0))
                    weights = F.softmax(
                        log_vals / self.temperature, dim=-1,
                    )
                    weights = weights * mask
                    weights = weights / weights.sum(
                        dim=1, keepdim=True,
                    ).clamp(min=1e-8)
                    level_out = (emb * weights.unsqueeze(-1)).sum(dim=1)
                else:
                    # ── Cross-level attention from coarser context ──
                    # Query: coarse context  →  (B, emb_dim)
                    q = self.cross_queries[level - 1](ctx)           # (B, emb_dim)
                    # Key: current level embeddings  →  (B, L, emb_dim)
                    k = self.cross_keys[level - 1](emb)              # (B, L, emb_dim)

                    # Scaled dot-product attention scores
                    attn_score = torch.bmm(
                        q.unsqueeze(1), k.transpose(1, 2),
                    ).squeeze(1)                                      # (B, L)
                    attn_score = attn_score / math.sqrt(self.emb_dim)

                    # Value-based bias from dense statistics
                    log_vals = torch.log1p(dense_vals.clamp(min=0))
                    value_bias = log_vals / self.temperature          # (B, L)

                    # Joint weighting: semantic attention + statistical bias
                    combined = attn_score + value_bias
                ...
                # ── Update coarse context for next level ──
                if level < self.num_levels - 1:
                    if ctx is None:
                        ctx = level_out
                    else:
                        ctx = self.ctx_fusions[level - 1](
                            torch.cat([ctx, level_out], dim=-1),
                        )

            # Per-level LayerNorm
            level_out = self.level_norms[level](level_out)
            outputs.append(level_out)  # (B, emb_dim)
...
class GroupNSTokenizer(nn.Module):
    ...
    Supports weighted pooling for (key, value) aligned features: when
    ``weighted_fid_config`` is configured and ``dense_feats`` is provided,
    the multi-valued int embedding is pooled with ``softmax(log1p(dense_vals))``
    weights instead of uniform mean.

    Supports hierarchical KV fusion (Step 2.b): when ``hierarchical_kv_config``
    is configured and ``hierarchical_fusion`` is provided, a chain of
    coarse→fine fid pairs is jointly processed with cross-level attention.
    """

    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 groups: List[List[int]], emb_dim: int, d_model: int,
                 emb_skip_threshold: int = 0,
                 weighted_fid_config: Optional[Dict[int, Tuple[int, int]]] = None,
                 softmax_temperature: float = 2.0,
                 hierarchical_kv_config: Optional[Dict[int, Dict]] = None,
                 hierarchical_fusion: Optional['HierarchicalKVFusion'] = None) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.weighted_fid_config = weighted_fid_config or {}
        self.softmax_temperature = softmax_temperature
        self.hierarchical_kv_config = hierarchical_kv_config or {}
        self.hierarchical_fusion = hierarchical_fusion
        # Build a flat set of spec indices that belong to ANY hierarchical chain
        self._hierarchical_fids: set = set()
        for _chain_info in self.hierarchical_kv_config.values():
            self._hierarchical_fids.update(_chain_info.get('spec_indices', []))
    ...
    def forward(self, int_feats: torch.Tensor,
        ...
        Args:
            ...
            dense_feats: Optional (B, total_dense_dim) for weighted pooling.
        ...
        for group, proj in zip(self.groups, self.group_projs):
            fid_embs = []
            # ── Iterate with manual index to allow look-ahead for chains ──
            i = 0
            while i < len(group):
                fid_idx = group[i]

                # ── Step 2.b: hierarchical chain detected ──
                if (self.hierarchical_fusion is not None
                        and fid_idx in self.hierarchical_kv_config):
                    chain_info = self.hierarchical_kv_config[fid_idx]
                    chain_indices = chain_info['spec_indices']  # list of spec idx
                    chain_dense_offs = chain_info['dense_offsets']  # list of (off, len)

                    # Collect int / dense data and emb layers for the chain
                    chain_int = []
                    chain_dense = []
                    chain_emb = []
                    for _j, _spec_idx in enumerate(chain_indices):
                        _vs, _off, _len = self.feature_specs[_spec_idx]
                        chain_int.append(
                            int_feats[:, _off:_off + _len].long())
                        if dense_feats is not None and _j < len(chain_dense_offs):
                            _doff, _dlen = chain_dense_offs[_j]
                            chain_dense.append(
                                dense_feats[:, _doff:_doff + _dlen].float())
                        else:
                            chain_dense.append(
                                int_feats.new_zeros(int_feats.shape[0], _len))
                        _emb_real = self._emb_index[_spec_idx]
                        chain_emb.append(
                            self.embs[_emb_real] if _emb_real >= 0 else None)

                    # Hierarchical fusion → list of (B, emb_dim)
                    chain_outputs = self.hierarchical_fusion(
                        chain_int, chain_dense, chain_emb,
                    )
                    fid_embs.extend(chain_outputs)

                    # Skip processed chain fids
                    i += len(chain_indices)
                    continue

                # ── Standard single-fid processing ──
                ...
                if emb_real_idx == -1:
                    ...
                    elif (self.weighted_fid_config
                          and fid_idx in self.weighted_fid_config
                          and dense_feats is not None):
                        # ── Step 2.a: (key,value) weighted pooling ──
                        dense_offset, _ = self.weighted_fid_config[fid_idx]
                        dense_vals = dense_feats[:, dense_offset:dense_offset + length].float()
                        vals = int_feats[:, offset:offset + length].long()
                        emb_all = emb_layer(vals)
                        log_vals = torch.log1p(dense_vals.clamp(min=0))
                        weights = F.softmax(log_vals / self.softmax_temperature, dim=-1)
                        mask = (vals != 0).float()
                        weights = weights * mask
...
class RankMixerNSTokenizer(nn.Module):
    ...
    Supports weighted pooling for (key, value) aligned features: when
    ``weighted_fid_config`` maps a fid index to a ``(dense_offset, dense_length)``
    pair and ``dense_feats`` is provided, the multi-valued int embedding is
    pooled with ``softmax(log1p(dense_vals))`` weights instead of uniform mean.
    ...
    def __init__(
        ...
        softmax_temperature: float = 2.0,
        hierarchical_kv_config: Optional[Dict[int, Dict]] = None,
        hierarchical_fusion: Optional['HierarchicalKVFusion'] = None,
    ) -> None:
        ...
        Args:
            ...
            weighted_fid_config: Optional dict mapping fid spec index →
                ``(dense_offset, dense_length)`` for (key,value) weighted pooling.
                If None or if ``dense_feats`` is not provided at forward time,
                uniform mean pooling is used for all multi-valued features.
            hierarchical_kv_config: Optional dict mapping chain-start spec index →
                ``{'spec_indices': [...], 'dense_offsets': [(off,len),...]}``.
            hierarchical_fusion: Optional HierarchicalKVFusion instance for
                cross-level attention fusion (Step 2.b).
        """
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        ...
        logging.info(
            ...
            f"{', weighted_fids='+str(list(self.weighted_fid_config.keys())) if self.weighted_fid_config else ''}"
        )
    ...
    def forward(self, int_feats: torch.Tensor,
        ...
        Args:
            int_feats: (B, total_int_dim) concatenated integer features.
            dense_feats: Optional (B, total_dense_dim) for (key,value)
                weighted pooling. Only used when ``weighted_fid_config``
        ...
        for group in self.groups:
            ...
            while i < len(group):
                ...
                if (self.hierarchical_fusion is not None
                        and fid_idx in self.hierarchical_kv_config):
                    chain_info = self.hierarchical_kv_config[fid_idx]
                    chain_indices = chain_info['spec_indices']
                    chain_dense_offs = chain_info['dense_offsets']
                ...
                if emb_real_idx == -1:
                    ...
                    if length == 1:
                        ...
                        vals = int_feats[:, offset:offset + length].long()
                        emb_all = emb_layer(vals)
                        # log1p compression → temperature-scaled softmax → mask padding positions
                        log_vals = torch.log1p(dense_vals.clamp(min=0))
                        weights = F.softmax(log_vals / self.softmax_temperature, dim=-1)  # (B, length)
                        mask = (vals != 0).float()          # (B, length)
                        weights = weights * mask
                        weights = weights / weights.sum(dim=1, keepdim=True).clamp(min=1e-8)
                        fid_emb = (emb_all * weights.unsqueeze(-1)).sum(dim=1)  # (B, emb_dim)
                    else:
                        vals = int_feats[:, offset:offset + length].long()
                        emb_all = emb_layer(vals)
                        mask = (vals != 0).float().unsqueeze(-1)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # ── Step 2.a update: temperature scaling for KV weighted pooling ──
        kv_softmax_temperature: float = 2.0,
    ) -> None:
        super().__init__()
        ...
        # ── Step 2.a: (key,value) weighted pooling config for fids 89/90/91 ──
        # These 3 user_int features have matching user_dense columns (dim=10
        # each) forming aligned (entity_id, statistic) pairs.  The dense values
        # are treated as softmax weights for the int embeddings.
        # user_dense layout (last 30 dims = fid 89/90/91, 10 dims each):
        #   [61:256, 62:6, 63:19, 64:26, 65:111, 66:150, 87:320, 89:10, 90:10, 91:10]
        #   non_aligned_dim = 256+6+19+26+111+150+320 = 888
        # user_int spec positions for fid 89/90/91 are determined by the
        # schema.json ordering and the FeatureSchema offset accumulation.
        # We detect them by matching the cumulative user_int offset against
        # positions 25/26/27 in the 46-entry user_int feature list.
        _kv_weighted_config: Dict[int, Tuple[int, int]] = {}
        if user_dense_dim >= 30:
            # Build fid spec index → (dense_offset, dense_length) for the 3 pairs
            _non_aligned_dim = user_dense_dim - 30  # everything before fid 89
            for _pair_i, _int_spec_pos in enumerate([25, 26, 27]):  # fid 89, 90, 91
                if _int_spec_pos < len(user_int_feature_specs):
                    _dense_off = _non_aligned_dim + _pair_i * 10
                    _kv_weighted_config[_int_spec_pos] = (_dense_off, 10)
        # ── End Step 2.a config ──

        # ── Step 2.b: hierarchical KV fusion config for fids 62→63→64→65→66 ──
        # 5-level pyramid from coarse (L=6) to finest (L=150) granularity.
        # Each level is an (int, dense) aligned pair with matching lengths.
        # Coarser context guides finer pooling via cross-level attention.
        # user_dense layout: [61:256, 62:6, 63:19, 64:26, 65:111, 66:150, 87:320, ...]
        # user_int spec indices for fid 62/63/64/65/66 = 17/18/19/20/21 (0-indexed).
        _hierarchical_kv_config: Dict[int, Dict] = {}
        _hierarchical_fusion: Optional[HierarchicalKVFusion] = None
        if user_dense_dim >= 568:  # need at least up to fid 66 (offset 418+150=568)
            _chain_spec_indices = [17, 18, 19, 20, 21]   # fids 62, 63, 64, 65, 66
            _chain_dense_offsets: List[Tuple[int, int]] = [
                (256, 6),    # fid 62
                (262, 19),   # fid 63
                (281, 26),   # fid 64
                (307, 111),  # fid 65
            ...
            # Validate chain indices are within bounds
            if all(_si < len(user_int_feature_specs) for _si in _chain_spec_indices):
                _hierarchical_kv_config[_chain_spec_indices[0]] = {
                    'spec_indices': _chain_spec_indices,
                    'dense_offsets': _chain_dense_offsets,
                }
                _hierarchical_fusion = HierarchicalKVFusion(
                    emb_dim=emb_dim,
                    num_levels=len(_chain_spec_indices),
                    temperature=kv_softmax_temperature,
                )
                logging.info(
                    f"Step 2.b HierarchicalKVFusion enabled: "
                    f"chain={_chain_spec_indices} (fids 62→63→64→65→66), "
                    f"dense_offsets={_chain_dense_offsets}, "
                    f"temperature={kv_softmax_temperature}"
                )
        # ── End Step 2.b config ──
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                weighted_fid_config=_kv_weighted_config,
                softmax_temperature=kv_softmax_temperature,
                hierarchical_kv_config=_hierarchical_kv_config,
                hierarchical_fusion=_hierarchical_fusion,
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: grouped projection (with optional weighted pooling)
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats,
                                         inputs.user_dense_feats)   # (B, num_user_groups, D)
