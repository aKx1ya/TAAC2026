...
class HashEmbedding(nn.Module):
    ...
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        pad_mask = (ids == 0)
        h0 = torch.where(pad_mask, 0, (ids.abs() % (self.num_buckets - 1)) + 1)
        result = self.embs[0](h0)
        for i in range(1, self.num_hashes):
            h = torch.where(pad_mask, 0, (ids.abs() * self.HASH_PRIMES[i - 1]) % (self.num_buckets - 1) + 1)
...
class RoPEMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if attn_mask is not None:
            # Support both 2D (Lq, Lk) and 3D (B, Lq, Lk) additive float masks.
            if attn_mask.dim() == 2:
                bool_attn = (attn_mask == 0)
                bool_attn = bool_attn.unsqueeze(0).unsqueeze(0).expand(B, self.num_heads, Lq, Lk)
            else:  # 3D: (B, Lq, Lk), pre-built additive mask
                bool_attn = (attn_mask == 0).unsqueeze(1).expand(B, self.num_heads, Lq, Lk)
            if use_float_mask:
...
class LongerEncoder(nn.Module):
    ...
    def forward(
        ...
        if L > self.top_k:
            ...
            # 4. Cross Attention with optional causal mask.
            # Build rectangular causal mask: Q at original position p can only
            # attend to K positions ≤ p (prevents future-information leakage
            # within the top_k window).
            causal_mask = None
            if self.causal:
                k_pos = torch.arange(L, device=x.device).view(1, 1, L)       # (1, 1, L)
                q_pos = q_pos_indices.unsqueeze(-1)                           # (B, top_k, 1)
                causal_mask = torch.where(
                    k_pos <= q_pos,
                    torch.tensor(0.0, device=x.device),
                    torch.tensor(float('-inf'), device=x.device),
                )  # (B, top_k, L), additive: 0=attend, -inf=mask
...
class GroupNSTokenizer(nn.Module):
    ...
    Supports hierarchical sparse-dense fusion for pyramid-style aligned
    (key,value) feature chains when ``hierarchical_fusion`` is provided.
    """
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 hierarchical_fusion: Optional['HierarchicalSparseDenseFusion'] = None,
                 hierarchical_chain_info: Optional[Dict[int, Dict]] = None) -> None:
        super().__init__()
        ...
        self.hierarchical_chain_info = hierarchical_chain_info or {}

        # Build flat set of spec indices that belong to ANY hierarchical chain
        self._hierarchical_fids: set = set()
        for _chain_info in self.hierarchical_chain_info.values():
        ...
        for group in groups:
            ...
            while i < len(group):
                fid_idx = group[i]
                if fid_idx in self._hierarchical_fids and fid_idx not in self.hierarchical_chain_info:
                    i += 1
                    continue
                if self.hierarchical_fusion is not None and fid_idx in self.hierarchical_chain_info:
                    chain_len = len(self.hierarchical_chain_info[fid_idx]['spec_indices'])
                    out_fids += chain_len
                    i += chain_len
                    continue
                out_fids += 1
            ...
            self.group_projs.append(nn.Sequential(
                nn.Linear(out_fids * emb_dim, d_model),
                nn.LayerNorm(d_model),
            ))
    ...
    def forward(self, int_feats: torch.Tensor,
        ...
        Args:
            ...
            dense_feats: Optional (B, total_dense_dim) for hierarchical fusion.
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            while i < len(group):
                ...
                # Skip fids already processed as part of a hierarchical chain
                if (fid_idx in self._hierarchical_fids
                        and fid_idx not in self.hierarchical_chain_info):
                    i += 1
                    continue

                # ── Hierarchical chain detected ──
                if (self.hierarchical_fusion is not None
                        and fid_idx in self.hierarchical_chain_info):
                    chain_info = self.hierarchical_chain_info[fid_idx]
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        hash_bucket_size: int = 0,
        hierarchical_fusion: Optional['HierarchicalSparseDenseFusion'] = None,
        hierarchical_chain_info: Optional[Dict[int, Dict]] = None,
    ) -> None:
        ...
        Args:
            ...
            hierarchical_fusion: Optional HierarchicalSparseDenseFusion for
                pyramid-style (key,value) feature chains.
            hierarchical_chain_info: Optional dict mapping first spec index →
        ...
        logging.info(
            ...
            f"{', hierarchical_chain=' + str(list(self.hierarchical_chain_info.keys())) if self.hierarchical_chain_info else ''}"
        )
...
class HierarchicalSparseDenseFusion(nn.Module):
    """Hierarchical sparse-dense fusion for pyramid-style (key,value) features.

    Fuses 5 aligned (int, dense) pairs (fids 62→66) with coarse→fine
    cross-level attention, using v9's sign-aware log1p weighting.

    Level 0 (coarsest, L=6):  sign-aware weighted mean pooling → ctx₀
    Level l > 0:              coarse ctx as Query, current emb as Key
                              score = q·k/√d + log1p(|dense|)*sign(dense)/T
                              ctx ← FusionMLP([ctx, level_out])

    Outputs 5 per-level vectors (B, emb_dim) preserving multi-granularity.
    Uses the NS tokenizer's embedding tables — no separate embeddings.
    """
    ...
    def __init__(
        ...
        # Cross-level attention: coarse context → query, current level → key
        ...
        # Context fusion: [coarse_ctx; level_out] → new coarse context
    ...
    def forward(
        self,
        int_list: List[torch.Tensor],
        dense_list: List[torch.Tensor],
        emb_layers: List[Optional[nn.Embedding]],
    ) -> List[torch.Tensor]:
        """Execute hierarchical fusion over one (int,dense) aligned chain.

        Args:
            int_list:   [ (B, L₀), ..., (B, L₄) ]  raw integer feature values.
            dense_list: [ (B, L₀), ..., (B, L₄) ]  corresponding float values.
            emb_layers: [ emb|None, ... ]  per-level embedding tables.

        Returns:
            List of (B, emb_dim) — one context-aware embedding per level.
        """
        outputs: List[torch.Tensor] = []
        ctx: Optional[torch.Tensor] = None

        for level in range(self.num_levels):
            int_vals = int_list[level]
            dense_vals = dense_list[level]
            ...
            if emb_layer is None:
                ...
                if level == 0:
                    # ── Coarsest level: v9-style sign-aware weighted pooling ──
                    weights = torch.log1p(dense_vals.abs()) * dense_vals.sign()
                    weights = weights * mask
                    count = mask.sum(dim=1, keepdim=True).clamp(min=1.0)
                    level_out = (emb * weights.unsqueeze(-1)).sum(dim=1) / count
                else:
                    ...
                    ).squeeze(1) / math.sqrt(self.emb_dim)  # (B, L)

                    # v9-style statistical bias with sign preservation
                    value_bias = torch.log1p(dense_vals.abs()) * dense_vals.sign()
                    value_bias = value_bias / self.temperature

                    # Joint weighting: semantic + statistical
                    combined = attn_score + value_bias
                    combined = combined.masked_fill(mask == 0, float('-inf'))
                    weights = F.softmax(combined, dim=-1)
                    weights = torch.nan_to_num(weights, nan=0.0)

                    level_out = (emb * weights.unsqueeze(-1)).sum(dim=1)  # (B, emb_dim)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Fid list for dynamic feature lookup (prevents hardcoded spec indices)
        user_int_fids: Optional[List[int]] = None,
        # Dense column specs for dynamic offset computation
        user_dense_cols: Optional[List[List[int]]] = None,
    ) -> None:
        super().__init__()
        ...
        # ================== Hierarchical KV Fusion Config (fids 62→66) ==================
        # 5-level pyramid from coarse (L=6, vocab=11) to finest (L=150, vocab=1403).
        # Spec indices AND dense offsets are both resolved dynamically from schema
        # metadata so that reordering/insertion of features does not silently break.
        _hierarchical_chain_info: Dict[int, Dict] = {}
        _hierarchical_fusion: Optional[HierarchicalSparseDenseFusion] = None

        # Build dense offset map from schema metadata (used for dynamic offset
        # computation both here and in _make_user_dense_proj_input).
        _dense_offset_map: Dict[int, Tuple[int, int]] = {}
        self._user_dense_pair_start: int = 0
        self._user_dense_pair_end: int = 0
        if user_dense_cols:
            _running_offset = 0
            for _fid, _dim in user_dense_cols:
                _dense_offset_map[_fid] = (_running_offset, _dim)
                _running_offset += _dim

        if user_int_fids and user_dense_cols:
            _chain_fids = [62, 63, 64, 65, 66]
            _has_all_dense = all(fid in _dense_offset_map for fid in _chain_fids)
            _fid_to_spec = {fid: i for i, fid in enumerate(user_int_fids)}
            _has_all_int = all(fid in _fid_to_spec for fid in _chain_fids)

            if _has_all_int and _has_all_dense:
                _chain_spec_indices = [
                    _fid_to_spec[fid] for fid in _chain_fids
                ]
                _chain_dense_offsets: List[Tuple[int, int]] = [
                    _dense_offset_map[fid] for fid in _chain_fids
                ]
                if all(_si < len(user_int_feature_specs) for _si in _chain_spec_indices):
                    _hierarchical_chain_info[_chain_spec_indices[0]] = {
                        'spec_indices': _chain_spec_indices,
                        'dense_offsets': _chain_dense_offsets,
                    }
                    _hierarchical_fusion = HierarchicalSparseDenseFusion(
                        emb_dim=emb_dim,
                        num_levels=len(_chain_spec_indices),
                        temperature=2.0,
                    )
                    # Compute dynamic dense pair boundaries for cropping
                    _start_off, _ = _chain_dense_offsets[0]
                    _end_off, _end_dim = _chain_dense_offsets[-1]
                    self._user_dense_pair_start = _start_off
                    self._user_dense_pair_end = _end_off + _end_dim
                    logging.info(
                        f"HierarchicalSparseDenseFusion enabled: "
                        f"fids={_chain_fids} -> spec_indices={_chain_spec_indices}, "
                        f"dense_offsets={_chain_dense_offsets}, "
                        f"pair_range=[{self._user_dense_pair_start}, {self._user_dense_pair_end})"
                    )
            else:
                logging.warning(
                    f"HierarchicalSparseDenseFusion disabled: "
                    f"some chain fids {_chain_fids} not found in "
                    f"user_int_fids or user_dense_cols"
                )
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                hierarchical_chain_info=_hierarchical_chain_info,
            )
        ...
        # Fid 62-66 dense columns are excluded — they are handled by
        # HierarchicalSparseDenseFusion inside the NS tokenizer.
        self.has_user_dense = user_dense_dim > 0
        self.user_dense_proj_dim = user_dense_dim
        if self._user_dense_pair_end > 0:
            self.user_dense_proj_dim = (
                user_dense_dim - (self._user_dense_pair_end - self._user_dense_pair_start)
            )
    ...
    def _make_user_dense_proj_input(self, user_dense_feats: torch.Tensor) -> torch.Tensor:
        ...
        These columns are already consumed by HierarchicalSparseDenseFusion inside
        the NS tokenizer; feeding them again through user_dense_proj would
        double-count the same information.
        """
        if self._user_dense_pair_end == 0 or user_dense_feats.shape[1] < self._user_dense_pair_end:
            return user_dense_feats
        return torch.cat([
            user_dense_feats[:, :self._user_dense_pair_start],
            user_dense_feats[:, self._user_dense_pair_end:],
        ], dim=1)
    ...
    def _make_user_dense_token(
        ...
        """Builds the single user dense NS token via linear projection."""
        dense_proj_input = self._make_user_dense_proj_input(user_dense_feats)
        return F.silu(self.user_dense_proj(dense_proj_input)).unsqueeze(1)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: grouped projection (user side gets dense for hierarchical fusion)
        user_ns = self.user_ns_tokenizer(
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = self._make_user_dense_token(inputs.user_dense_feats)
