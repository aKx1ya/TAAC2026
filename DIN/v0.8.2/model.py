...
class SparseDensePairEncoder(nn.Module):
    """Complete sparse-dense pair encoder for fids 62-66.

    Replaces the simpler ``UserSparseDensePairResidual`` with a comprehensive
    design that treats dense values as first-class signals rather than mere
    modulation weights.

    Four core improvements
    ----------------------
    1. **Dense Value Embedding** — per-position ``Linear(1, emb_dim)`` gives
       dense values an independent encoding channel, parallel to the int
       embedding lookup.  The two paths are fused via a learned per-position
       sigmoid gate.

    2. **Multi-Granularity Pooling** — each fid produces *two* ``emb_dim``
       vectors instead of one:
       - *coarse*: log1p-weighted mean  (overall preference tendency)
       - *fine*:   max-abs-dense selection  (strongest preference peak)

    3. **Learnable Residual Gate** — a scalar parameter (initialised so that
       :math:`\\sigma(0)=0.5`) controls how much pair information is injected
       into the ``user_dense_token``.  The model can learn to trust (or
       down-weight) the paired signal dynamically.

    4. **Optional Cross-FID Attention** — lightweight 1-head self-attention
       across the five fid representations, letting "category preference"
       and "author preference" exchange information before projection.
    """
    ...
    def __init__(
        ...
        emb_dim: int,
        d_model: int,
        use_cross_fid_attn: bool = True,
        dropout: float = 0.0,
    ) -> None:
        """Initialises the enhanced pair encoder.

        Args:
            pair_specs:
                ``[(fid, vocab_size, int_offset, dense_offset, length), ...]``.
            emb_dim:
                Per-fid embedding / value-embedding dimension.
            d_model:
                Output dimension (added as residual to user_dense_token).
            use_cross_fid_attn:
                Whether to apply a lightweight 1-head self-attention across
                the five fid representations before final projection.
            dropout:
                Dropout rate for the cross-fid attention (if enabled).
        """
        super().__init__()
        self.pair_specs = pair_specs
        self.num_fids = len(pair_specs)
        self.emb_dim = emb_dim
        self.d_model = d_model

        # ---- Per-fid int embedding tables ----
        self.embs = nn.ModuleList([
            nn.Embedding(int(vs) + 1, emb_dim, padding_idx=0)
            for _, vs, _, _, _ in pair_specs
        ])

        # ---- Per-fid dense value embedding (independent channel) ----
        # Each scalar dense value is projected to emb_dim via a shared
        # per-fid linear layer.  This lets the model learn a continuous
        # value→vector mapping (e.g. "score 0.8" → a specific direction).
        self.dense_projs = nn.ModuleList([
            nn.Linear(1, emb_dim)
            for _ in range(self.num_fids)
        ])

        # ---- Per-fid gate networks (fuse int_emb and dense_emb) ----
        # Input: concat(int_emb, dense_emb) = 2*emb_dim
        # Output: emb_dim gating vector via sigmoid
        ...
        # ---- Per-fid output LayerNorm ----
        self.fid_norms = nn.ModuleList([
            nn.LayerNorm(emb_dim * 2)  # coarse + fine
            for _ in range(self.num_fids)
        ])

        # ---- Cross-FID self-attention (optional) ----
        self.use_cross_fid_attn = use_cross_fid_attn
        if use_cross_fid_attn:
            ...
            self.cross_attn_norm = nn.LayerNorm(emb_dim * 2)

        # ---- Final projection to d_model ----
        # 5 fids × 2 granularities × emb_dim  →  d_model
        self.proj = nn.Sequential(
            nn.Linear(self.num_fids * emb_dim * 2, d_model),
            nn.LayerNorm(d_model),
        )

        # ---- Learnable residual gate ----
        # Initialised at 0 → σ(0) ≈ 0.5, giving equal trust to both paths.
        self.residual_gate_logit = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(
        ...
        """Encodes the five paired (int, dense) features into a d_model residual.

        Args:
            user_int_feats:   ``(B, total_int_dim)``.
            user_dense_feats: ``(B, total_dense_dim)``.

        Returns:
            Residual tensor of shape ``(B, d_model)``, ready to be *added*
            (with learned gate) to the ordinary ``user_dense_token``.
        """
        fid_outputs: List[torch.Tensor] = []

        for i, (emb, dense_proj, gate_net, fid_norm,
                (fid, _, int_offset, dense_offset, L)) in enumerate(
            zip(self.embs, self.dense_projs, self.fusion_gates,
                self.fid_norms, self.pair_specs)
        ):
            B = user_int_feats.shape[0]

            # ---- 1. Int path: embedding lookup ----
            ids = user_int_feats[:, int_offset:int_offset + L].long()      # (B, L)
            int_emb = emb(ids)                                              # (B, L, emb_dim)

            # ---- 2. Dense path: value embedding (independent channel) ----
            dense_vals = user_dense_feats[:, dense_offset:dense_offset + L]  # (B, L)
            # log1p smoothing for long-tail robustness (proven effective)
            dense_smooth = torch.log1p(dense_vals.abs()) * dense_vals.sign()
            dense_emb = dense_proj(dense_smooth.unsqueeze(-1))              # (B, L, emb_dim)

            # ---- 3. Gated fusion of int and dense paths ----
            gate_input = torch.cat([int_emb, dense_emb], dim=-1)           # (B, L, 2*emb_dim)
            gate = gate_net(gate_input)                                     # (B, L, emb_dim)
            fused = gate * int_emb + (1.0 - gate) * dense_emb              # (B, L, emb_dim)

            # Padding mask (int ID == 0 means padding)
            mask = (ids != 0).to(dtype=fused.dtype).unsqueeze(-1)          # (B, L, 1)
            count = mask.sum(dim=1).clamp(min=1.0)                          # (B, 1)

            # ---- 4a. Coarse pooling: log1p-weighted mean ----
            weights = torch.log1p(dense_vals.abs()) * dense_vals.sign()
            weights = weights.unsqueeze(-1)                                 # (B, L, 1)
            coarse = (fused * weights * mask).sum(dim=1) / count           # (B, emb_dim)

            # ---- 4b. Fine pooling: max-abs-dense signal selection ----
            abs_dense = dense_vals.abs() * mask.squeeze(-1)                # (B, L)
            # If all positions are padding, argmax returns 0 (valid — mask
            # will zero the output below).
            max_idx = abs_dense.argmax(dim=1)                               # (B,)
            fine = fused[torch.arange(B, device=fused.device), max_idx]    # (B, emb_dim)
            # Zero out samples where all positions were padding
            has_valid = (count.squeeze(-1) > 0).to(dtype=fused.dtype).unsqueeze(-1)
            fine = fine * has_valid

            # ---- 5. Per-fid output: concat coarse + fine ----
            fid_out = torch.cat([coarse, fine], dim=-1)                    # (B, 2*emb_dim)
            fid_out = fid_norm(fid_out)
            fid_outputs.append(fid_out)

        # ---- 6. Optional cross-FID self-attention ----
        if self.use_cross_fid_attn:
            # (B, num_fids, 2*emb_dim)
            fid_stack = torch.stack(fid_outputs, dim=1)
            attn_out, _ = self.cross_attn(fid_stack, fid_stack, fid_stack)
            fid_stack = self.cross_attn_norm(fid_stack + attn_out)
            fid_outputs = [fid_stack[:, i, :] for i in range(self.num_fids)]

        # ---- 7. Final projection to d_model ----
        residual = F.silu(self.proj(torch.cat(fid_outputs, dim=-1)))      # (B, d_model)

        # ---- 8. Learnable residual gate ----
        gate_weight = torch.sigmoid(self.residual_gate_logit)
        return gate_weight * residual


# ── Backward-compatible alias ────────────────────────────────────────────────
# Kept so that checkpoint loading / inference code that references
# ``UserSparseDensePairResidual`` does not break.
UserSparseDensePairResidual = SparseDensePairEncoder
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # SparseDensePairEncoder options
        use_pair_cross_attn: bool = True,
        ...
        self.user_sparse_dense_pair_residual: Optional[SparseDensePairEncoder] = None
        if self.has_user_dense:
            ...
            if user_sparse_dense_pair_specs:
                self.user_sparse_dense_pair_residual = SparseDensePairEncoder(
                    pair_specs=user_sparse_dense_pair_specs,
                    emb_dim=emb_dim,
                    d_model=d_model,
                    use_cross_fid_attn=use_pair_cross_attn,
                    dropout=pair_dropout,
                )
                logging.info(
                    f"SparseDensePairEncoder: {len(user_sparse_dense_pair_specs)} fids, "
                    f"cross_attn={use_pair_cross_attn}"
                )
    ...
    def _make_user_dense_token(
        ...
        """Builds the single user dense NS token.

        The ordinary dense features (excluding fids 62-66) are projected
        to d_model.  The paired (int, dense) features 62-66 are encoded
        by ``SparseDensePairEncoder`` and added as a gated residual.
        The gate is a learnable scalar inside the encoder, so no extra
