...
_FALLBACK_MODEL_CFG = {
    ...
    # SparseDensePairEncoder (v2)
    "use_pair_cross_attn": True,
    "pair_dropout": 0.0,
}
...
def build_model(
    ...
    model = PCVRHyFormer(
        ...
        use_pair_cross_attn=model_cfg.get('use_pair_cross_attn', True),
        pair_dropout=model_cfg.get('pair_dropout', 0.0),
        **model_cfg,
    ).to(device)
...
def load_model_state_strict(
    ...
    """Load state_dict with strict matching, falling back gracefully.

    Tries strict loading first.  If the checkpoint was produced by an older
    version of the model (e.g. before SparseDensePairEncoder was introduced),
    falls back to non-strict loading and logs missing / unexpected keys.
    """
    state_dict = torch.load(ckpt_path, map_location=device)

    # Try strict first (preferred for matching model/checkpoint versions).
    try:
        model.load_state_dict(state_dict, strict=True)
        logging.info("Checkpoint loaded in strict mode — model and weights match exactly.")
        return
    except RuntimeError:
        logging.warning(
            "Strict load failed — checkpoint may be from an older model version. "
            "Falling back to non-strict mode (new layers will be randomly initialised)."
        )

    # Non-strict fallback: load what we can, report the rest.
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        logging.warning(
            f"Missing keys ({len(missing)}): new layers not present in checkpoint. "
            f"First 5: {missing[:5]}"
        )
    if unexpected:
        logging.warning(
            f"Unexpected keys ({len(unexpected)}): old layers not used by current model. "
            f"First 5: {unexpected[:5]}"
        )
    logging.info(
        f"Checkpoint loaded non-strictly: "
        f"loaded={len(state_dict) - len(unexpected)}, "
