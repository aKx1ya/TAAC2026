...
class PCVRHyFormerRankingTrainer:
    ...
    # ------------------------------------------------------------------
    # Epoch-level checkpoint helpers (keep best + N following epochs)
    # ------------------------------------------------------------------

    def _build_epoch_dir_name(self, epoch: int, is_best: bool = False) -> str:
        """Build a checkpoint sub-directory name keyed by epoch number.

        Example: ``epoch5.layer=2.head=4.hidden=64[.best_model]``.
        """
        parts = [f"epoch{epoch}"]
        for key in ("layer", "head", "hidden"):
            if key in self.ckpt_params:
    ...
    def _save_epoch_checkpoint(self, epoch: int, is_best: bool = False) -> str:
        """Save model.pt + sidecar files under an epoch-named directory.

        Args:
            epoch: current epoch number.
            is_best: if True, append ``.best_model`` suffix to the directory
                name and delete any previously-existing ``.best_model`` dirs.

        Returns:
            Absolute path of the checkpoint directory.
        """
        if is_best:
            self._remove_old_epoch_best_dirs()
        dir_name = self._build_epoch_dir_name(epoch, is_best=is_best)
        ckpt_dir = os.path.join(self.save_dir, dir_name)
        os.makedirs(ckpt_dir, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(ckpt_dir, "model.pt"))
        self._write_sidecar_files(ckpt_dir)
    ...
    def _remove_old_epoch_best_dirs(self) -> None:
        """Delete stale ``epoch*.best_model`` directories."""
        pattern = os.path.join(self.save_dir, "epoch*.best_model")
        for old_dir in glob.glob(pattern):
            shutil.rmtree(old_dir)
            logging.info(f"Removed old epoch best_model dir: {old_dir}")

    def _cleanup_epoch_checkpoints(self) -> None:
        """Remove epoch checkpoints that are outside the protected window.

        Protected epochs: ``best_epoch``, ``best_epoch + 1``, ``best_epoch + 2``.
        Checkpoints named with the ``.best_model`` suffix are always kept
        (they represent the single best epoch and are already managed by
        ``_remove_old_epoch_best_dirs``).
        """
        best_epoch = self.early_stopping.best_epoch
        if best_epoch is None:
            return
        protected = {best_epoch, best_epoch + 1, best_epoch + 2}
        pattern = os.path.join(self.save_dir, "epoch*")
        for ckpt_dir in glob.glob(pattern):
            basename = os.path.basename(ckpt_dir)
            # Directories with .best_model suffix are managed separately.
            if basename.endswith(".best_model"):
                continue
            # Extract epoch number from "epoch{N}[.layer=...]"
            try:
                epoch_str = basename.replace("epoch", "").split(".")[0]
                epoch_num = int(epoch_str)
            except (ValueError, IndexError):
                continue
            if epoch_num not in protected:
                shutil.rmtree(ckpt_dir)
                logging.info(
                    f"Removed non-protected epoch checkpoint: {ckpt_dir}"
                )
    ...
    def _handle_validation_result(
        ...
        Args:
            total_step: current global step count.
            val_auc: validation AUC (higher is better).
            val_logloss: validation binary logloss.
            epoch: current epoch number, forwarded to EarlyStopping so that
                ``best_epoch`` can be recorded for epoch-checkpoint cleanup.
        """
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    self._handle_validation_result(total_step, val_auc, val_logloss, epoch=epoch)
            ...
            # Save every epoch checkpoint (EMA weights if available).
            if self.ema is not None:
                self.ema.apply_shadow(self.model)
            is_best_epoch = (
                self.early_stopping.best_epoch is not None
                and epoch == self.early_stopping.best_epoch
            )
            self._save_epoch_checkpoint(epoch, is_best=is_best_epoch)
            if self.ema is not None:
                self.ema.restore(self.model)
            # Keep only best_epoch + 2 following epochs.
            self._cleanup_epoch_checkpoints()
        ...
        # Final cleanup after training finishes (all epochs or early stop).
        self._cleanup_epoch_checkpoints()
        logging.info(
            f"Training finished. Best epoch: {self.early_stopping.best_epoch}, "
            f"Best AUC: {self.early_stopping.best_score:.6f}"
        )
