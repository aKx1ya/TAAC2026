...
def build_model(
    ...
    ue_int_offset: int = -1
    ue_int_vocab: int = 0
    no_ue_token = train_config.get("no_ue_token", False) if train_config else False
    if not no_ue_token:
        try:
            ...
            logging.info(
                f"UE Token enabled: fid=97, int_offset={ue_int_offset}, "
                f"vocab={ue_int_vocab}"
            )
        except Exception as e:
            logging.warning(f"UE Token disabled: cannot resolve fid 97 ({e})")
    else:
        logging.info("UE Token disabled via train_config no_ue_token=True")
