...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        timestamp_threshold: Optional[int] = None,
    ) -> None:
        """
        Args:
            ...
            is_training: if True, derive ``label`` from ``label_type == 2``;
                if False, return an all-zeros label column.  When
                ``timestamp_threshold`` is set, ``is_training`` also determines
                which side of the threshold to keep (True = older samples).
            timestamp_threshold: if set, filter samples by their ``timestamp``
                column: when ``is_training`` is True, samples with ``timestamp <
                threshold`` are kept (train split); when ``is_training`` is
                False, samples with ``timestamp >= threshold`` are kept (valid
                split).  ``None`` disables filtering and returns all samples.
        """
        super().__init__()
        ...
        self._timestamp_threshold = timestamp_threshold
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        for file_path, rg_idx, _ in rg_list:
            ...
            for raw_batch in pf.iter_batches(batch_size=self.batch_size, row_groups=[rg_idx]):
                batch_dict = self._convert_batch(raw_batch)

                # ---- Timestamp-based train/valid filtering ----
                if self._timestamp_threshold is not None:
                    ts = batch_dict['timestamp'].numpy()
                    if self.is_training:
                        keep_mask = ts < self._timestamp_threshold
                    else:
                        keep_mask = ts >= self._timestamp_threshold
                    if not keep_mask.any():
                        continue  # entire batch falls on the wrong side
                    if not keep_mask.all():
                        batch_dict = self._filter_batch_dict(batch_dict, keep_mask)
                # ----------------------------------------------------
    ...
    def _filter_batch_dict(
        self,
        batch_dict: Dict[str, Any],
        keep_mask: "npt.NDArray[np.bool_]",
    ) -> Dict[str, Any]:
        """Filter a batch dict by a boolean numpy mask, keeping only rows
        where ``keep_mask`` is True.

        Tensors are indexed with a torch bool mask; plain Python lists are
        filtered element-wise; all other values (e.g. ``_seq_domains``) are
        kept as-is.
        """
        ...
        for k, v in batch_dict.items():
            if isinstance(v, torch.Tensor):
                filtered[k] = v[mask_tensor]
            elif isinstance(v, list) and len(v) == len(keep_mask):
                # Only filter lists whose length matches the batch dimension
                # (per-row data like user_id).  Metadata lists like
                # _seq_domains have a different length and are kept as-is.
                filtered[k] = [v[i] for i in range(len(v)) if keep_mask[i]]
            else:
                filtered[k] = v
        return filtered
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # Always read labels from the data.  The ``is_training`` flag only
        # controls the timestamp-based train/valid split direction; labels
        # must be available for both training and validation evaluation.
...
def _scan_all_timestamps(data_dir: str) -> "npt.NDArray[np.int64]":
    """Scan the ``timestamp`` column from every ``*.parquet`` file under
    ``data_dir`` and return a sorted 1-D numpy array of all timestamps.

    This is a one-time O(N) pre-pass used to determine the time-based
    train / validation split threshold.
    """
    ...
    # Read only the timestamp column in bulk.
    all_ts: List["npt.NDArray[np.int64]"] = []
    for f in pq_files:
        pf = pq.ParquetFile(f)
        for batch in pf.iter_batches(columns=['timestamp'], batch_size=262144):
            ts = batch.column(0).to_numpy().astype(np.int64)
            all_ts.append(ts)
    if not all_ts:
        raise ValueError(f"No timestamp column found in {data_dir}")
    result = np.concatenate(all_ts)
    result.sort()
    logging.info(
        f"Scanned {len(result)} timestamps from {len(pq_files)} parquet file(s), "
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    The validation split is **time-based**: the ``timestamp`` column of every
    sample is scanned once, and the latest ``valid_ratio`` fraction (by time)
    is held out as the validation set.  This matches the TAAC competition
    setup where the held-out test set immediately follows the training period.
    ...
    # ---- Scan all timestamps & compute the time-split threshold ----
    all_ts = _scan_all_timestamps(data_dir)
    n_total = len(all_ts)
    n_valid = max(1, int(n_total * valid_ratio))
    threshold = int(all_ts[n_total - n_valid])  # first timestamp in valid window

    # Support train_ratio: truncate the training portion further.
    n_train_available = n_total - n_valid
    if train_ratio < 1.0:
        n_train_use = max(1, int(n_train_available * train_ratio))
        logging.info(
            f"train_ratio={train_ratio}: using first {n_train_use}/{n_train_available} "
            f"training samples")
    else:
        n_train_use = n_train_available

    logging.info(
        f"Time-based split: threshold ts={threshold}, "
        f"train≈{n_train_available} / valid≈{n_valid} samples "
        f"(valid_ratio={valid_ratio})")
    logging.info(
        f"Train ts range: [{all_ts[0]}, {all_ts[n_train_available - 1]}], "
        f"Valid ts range: [{all_ts[n_train_available]}, {all_ts[-1]}]")
    del all_ts

    # ---- Build datasets (both read ALL Row Groups; filtering is per-sample) ----
    ...
    logging.info(
        f"Parquet DataLoaders ready: batch_size={batch_size}, "
