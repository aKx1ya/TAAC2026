...
class PCVRHyFormerRankingTrainer:
    ...
    def _remove_old_best_dirs(self) -> None:
        """Keep only the top-K most-recent ``*.best_model`` directories on disk.

        Best-model directories are named ``global_step{N}...best_model`` and
        sorted by step number.  When more than ``save_top_k`` directories
        exist the oldest (lowest-step) ones are removed.
        """
        pattern = os.path.join(self.save_dir, "global_step*.best_model")
        all_best = sorted(glob.glob(pattern))
        if len(all_best) <= self.save_top_k:
            return
        excess = all_best[:len(all_best) - self.save_top_k]
        for old_dir in excess:
            shutil.rmtree(old_dir)
            logging.info(f"Removed old best_model dir: {old_dir}")

    def _save_periodic_checkpoint(self, epoch: int, global_step: int) -> str:
        """Save a regular (non-best) checkpoint at an epoch boundary.

        Uses EMA shadow weights when EMA is enabled.  Old periodic
        checkpoints exceeding ``save_top_k`` are automatically cleaned up.
        """
        if self.ema is not None:
            self.ema.apply_shadow(self.model)

        parts = [f"epoch{epoch}.global_step{global_step}"]
        for key in ("layer", "head", "hidden"):
            if key in self.ckpt_params:
                parts.append(f"{key}={self.ckpt_params[key]}")
        dir_name = ".".join(parts)
        ...
        logging.info(f"Saved periodic checkpoint to {ckpt_dir}/model.pt")
        self._cleanup_periodic_checkpoints()
        return ckpt_dir

    def _cleanup_periodic_checkpoints(self) -> None:
        """Keep only the most-recent K periodic (epoch-based) checkpoints."""
        pattern = os.path.join(self.save_dir, "epoch*.global_step*")
        all_periodic = sorted(glob.glob(pattern))
        if len(all_periodic) <= self.save_top_k:
            return
        excess = all_periodic[:len(all_periodic) - self.save_top_k]
        for old_dir in excess:
            shutil.rmtree(old_dir)
            logging.info(f"Removed old periodic checkpoint: {old_dir}")
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            if self.save_every_n_epochs > 0 and epoch % self.save_every_n_epochs == 0:
                self._save_periodic_checkpoint(epoch, total_step)
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        # Use the model's full seq_domains list (not the batch's _seq_domains)
        # to guard against batches where a domain could be missing after
        # timestamp-based filtering or buffer shuffling.
        seq_domains = self.model.seq_domains
        ...
        for domain in seq_domains:
            if domain in device_batch:
                ...
                # Domain missing from this batch (e.g., after filtering).
                # Derive batch size from user_int_feats and seq length from
                # another present domain, then fill with padding zeros.
                B = device_batch['user_int_feats'].shape[0]
                L = 256  # safe default
                for d in self.model.seq_domains:
                    if d in device_batch:
                        L = device_batch[d].shape[2]
                        break
                S = len(self.model._seq_vocab_sizes[domain])
                seq_data[domain] = torch.zeros(
                    B, S, L, dtype=torch.long, device=self.device)
                seq_lens[domain] = torch.zeros(
