...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--valid_ratio', type=float, default=0.02,
                        help='Fraction of latest data (by timestamp) used for validation')
    parser.add_argument('--eval_every_n_steps', type=int, default=0,
                        help='Run validation every N steps '
                             '(0 = only at the end of each epoch)')
    parser.add_argument('--save_top_k', type=int, default=1,
                        help='Keep top-K best checkpoints on disk (1 = only the best)')
    parser.add_argument('--save_every_n_epochs', type=int, default=0,
                        help='Save a full checkpoint every N epochs (0 = disabled)')
