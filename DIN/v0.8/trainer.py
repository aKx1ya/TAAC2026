...
class PCVRHyFormerRankingTrainer:
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            # Save per-epoch checkpoint regardless of whether it's the best AUC.
            epoch_dir_name = f"epoch{epoch}.global_step{total_step}"
            for key in ("layer", "head", "hidden"):
                if key in self.ckpt_params:
                    epoch_dir_name += f".{key}={self.ckpt_params[key]}"
            epoch_ckpt_dir = os.path.join(self.save_dir, epoch_dir_name)
            os.makedirs(epoch_ckpt_dir, exist_ok=True)
            torch.save(self.model.state_dict(), os.path.join(epoch_ckpt_dir, "model.pt"))
            self._write_sidecar_files(epoch_ckpt_dir)
            logging.info(f"Saved epoch checkpoint to {epoch_ckpt_dir}/model.pt")
