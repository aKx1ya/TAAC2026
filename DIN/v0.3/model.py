...
USER_DENSE_UE_END = 256     # fid 61 ends at offset 256 (same as PAIR_START)
...
class UETokenModule(nn.Module):
    """Builds a dedicated User Embedding token from fid 61 (dense) + fid 97 (int).

    fid 61 (256-dim user dense embedding) and fid 97 (vocab≈4 user attribute)
    are extracted from the raw feature tensors and fused into a single clean
    user-identity token.  This token lives on a separate path from NS tokens
    and is used for explicit UE × item interaction.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.ue_dense_proj = nn.Sequential(
            nn.Linear(ue_dense_dim, d_model),
            nn.LayerNorm(d_model),
        )
        self.ue_int_emb = nn.Embedding(ue_int_vocab + 1, d_model, padding_idx=0)
    ...
    def forward(self, ue_dense: torch.Tensor, ue_int: torch.Tensor) -> torch.Tensor:
        """Fuses fid 61 dense and fid 97 int into a single UE token.

        Args:
            ue_dense: (B, 256) raw fid 61 values.
            ue_int:   (B,) or (B, 1) raw fid 97 integer id.

        Returns:
            UE token of shape (B, d_model).
        """
        ue_d = F.silu(self.ue_dense_proj(ue_dense))             # (B, d_model)
        ue_i = self.ue_int_emb(ue_int.long().view(-1))           # (B, d_model)
        return self.fusion(torch.cat([ue_d, ue_i], dim=-1))      # (B, d_model)


class UEItemInteraction(nn.Module):
    """Explicit UE × item interaction via gated hadamard product.

    Computes UE_token ⊙ item_emb, projects through a small MLP with a
    learnable gate initialized near zero, and returns a gated residual.
    This allows the model to gradually learn how much explicit user-item
    matching signal to blend in.
    """
    ...
    def forward(self, ue_token: torch.Tensor, item_emb: torch.Tensor) -> torch.Tensor:
        """Produces a gated UE×item interaction residual.

        Args:
            ue_token: (B, d_model), the fused user-identity token.
            item_emb: (B, d_model), the pooled item representation.

        Returns:
            Gated interaction feature of shape (B, d_model).
        """
        pair = ue_token * item_emb                              # hadamard
        feat = self.proj(pair)                                   # (B, d_model)
        gate = torch.sigmoid(self.gate_logit).to(dtype=feat.dtype)
        return gate * feat
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # UE Token (fid 61 + 97 → explicit UE × item interaction)
        ue_int_offset: int = -1,
        ue_int_vocab: int = 0,
    ) -> None:
        super().__init__()
        ...
        self.ue_int_offset = ue_int_offset
        self.user_dense_proj_dim = user_dense_dim
        self.use_ue_token = ue_int_offset >= 0 and ue_int_vocab > 0
        if user_dense_dim >= USER_DENSE_PAIR_END:
            self.user_dense_proj_dim = (
                user_dense_dim - (USER_DENSE_PAIR_END - USER_DENSE_PAIR_START)
            )
        if self.use_ue_token:
            # Also strip fid 61 (offset 0..256) from the ordinary dense projection
            self.user_dense_proj_dim -= USER_DENSE_UE_END
            logging.info(
                f"UE Token enabled: fid 61 (0..{USER_DENSE_UE_END}) + "
                f"fid 97 (int_offset={ue_int_offset}, vocab={ue_int_vocab})"
            )
        ...
        # UE Token module (fid 61 dense + fid 97 int → explicit UE × item interaction)
        self.ue_token_module: Optional[UETokenModule] = None
        self.ue_item_interaction: Optional[UEItemInteraction] = None
        if self.use_ue_token:
            self.ue_token_module = UETokenModule(
                ue_dense_dim=USER_DENSE_UE_END,
                ue_int_vocab=ue_int_vocab,
                d_model=d_model,
            )
            self.ue_item_interaction = UEItemInteraction(d_model)
    ...
    def _init_params(self) -> None:
        ...
        if self.ue_token_module is not None:
            nn.init.xavier_normal_(self.ue_token_module.ue_int_emb.weight.data)
            self.ue_token_module.ue_int_emb.weight.data[0, :] = 0
    ...
    def reinit_high_cardinality_params(
        ...
        # UE token int embedding (fid 97, always low-cardinality → skip)
        if self.ue_token_module is not None:
            skip_count += 1
    ...
    def _make_user_dense_proj_input(self, user_dense_feats: torch.Tensor) -> torch.Tensor:
        """Drops fid62-66 (and fid 61 when UE token is enabled) before the
        ordinary dense-token projection to avoid double-counting."""
        if self.use_ue_token:
            # Strip fid 61 [0:256] AND fid 62-66 [256:568]
            if user_dense_feats.shape[1] < USER_DENSE_PAIR_END:
                if user_dense_feats.shape[1] < USER_DENSE_UE_END:
                    return user_dense_feats
                return user_dense_feats[:, USER_DENSE_UE_END:]
            return user_dense_feats[:, USER_DENSE_PAIR_END:]
        if user_dense_feats.shape[1] < USER_DENSE_PAIR_END:
            return user_dense_feats
        return torch.cat([
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1.5. UE Token: fid 61 (dense) + fid 97 (int) → dedicated user-identity token
        ue_token: Optional[torch.Tensor] = None
        if self.use_ue_token:
            ue_dense = inputs.user_dense_feats[:, :USER_DENSE_UE_END]
            ue_int = inputs.user_int_feats[:, self.ue_int_offset]
            ue_token = self.ue_token_module(ue_dense, ue_int)  # (B, d_model)
        ...
        # 7.5. UE × Item explicit interaction (gated hadamard-product residual)
        if self.use_ue_token and ue_token is not None:
            ue_item_feat = self.ue_item_interaction(ue_token, target_emb)
            output = output + ue_item_feat
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_ue_token:
            ue_dense = inputs.user_dense_feats[:, :USER_DENSE_UE_END]
            ue_int = inputs.user_int_feats[:, self.ue_int_offset]
            ue_token = self.ue_token_module(ue_dense, ue_int)
