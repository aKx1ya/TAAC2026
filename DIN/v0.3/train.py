...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--no_ue_token', action='store_true', default=False,
                        help='Disable the UE identity token (fid 61+97) and '
                             'UE×item explicit interaction')
...
def main() -> None:
    ...
    # UE Token (fid 61 dense + fid 97 int → explicit UE × item interaction)
    if not args.no_ue_token:
        try:
            ue_int_offset, ue_int_vocab = build_ue_spec(
                pcvr_dataset.user_int_schema,
                pcvr_dataset.user_int_vocab_sizes,
            )
            model_args["ue_int_offset"] = ue_int_offset
            model_args["ue_int_vocab"] = ue_int_vocab
            # Persist in args so they are saved to train_config.json for inference
            args.ue_int_offset = ue_int_offset
            args.ue_int_vocab = ue_int_vocab
            logging.info(f"UE Token enabled: fid=97, int_offset={ue_int_offset}, "
                         f"vocab={ue_int_vocab}")
        except Exception as e:
            logging.warning(f"UE Token disabled: cannot resolve fid 97 ({e})")
            args.ue_int_offset = -1
            args.ue_int_vocab = 0
    else:
        logging.info("UE Token disabled via --no_ue_token")
        args.ue_int_offset = -1
        args.ue_int_vocab = 0
