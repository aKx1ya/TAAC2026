...
class PCVRParquetDataset(IterableDataset):
    ...
    def _build_item_freq_map(self) -> None:
        ...
        Also builds ``self._freq_array`` for vectorised numpy lookup.
        """
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    # ── Share item freq/CTR maps from training to validation (Step 1a+1b) ──
    if train_dataset._item_dense_enabled:
        if train_dataset._item_freq_map is not None:
            valid_dataset.share_item_freq_map(
                train_dataset._item_freq_map,
                train_dataset._item_id_offset,
                train_dataset._item_dense_freq_offset,
            )
        if train_dataset._item_ctr_map is not None:
            valid_dataset.share_item_ctr_map(
                train_dataset._item_ctr_map,
                train_dataset._item_id_offset,
                train_dataset._item_dense_ctr_offset,
            )
