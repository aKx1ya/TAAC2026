...
UE_INT_FID = 97  # fid paired with fid 61 (dense) to form the UE identity token
...
def build_ue_spec(
    ...
    """Return ``(int_offset, vocab_size)`` for the UE int feature (fid 97)."""
    int_offset, int_length = user_int_schema.get_offset_length(fid)
    vs = max(user_int_vocab_sizes[int_offset:int_offset + int_length])
    return int_offset, int(vs)
...
def build_model(
    ...
    # ── UE Token (fid 61 dense + fid 97 int) ──
    no_ue_token = model_cfg.pop("no_ue_token", False)
    ue_int_offset = model_cfg.pop("ue_int_offset", -1)
    ue_int_vocab = model_cfg.pop("ue_int_vocab", 0)

    if not no_ue_token and ue_int_offset < 0:
        # train_config.json might not have the resolved values (legacy checkpoints).
        # Try to resolve from the dataset schema automatically.
        try:
            ue_int_offset, ue_int_vocab = build_ue_spec(
                dataset.user_int_schema,
                dataset.user_int_vocab_sizes,
            )
            logging.info(
                f"UE Token auto-resolved from schema: fid=97, "
                f"int_offset={ue_int_offset}, vocab={ue_int_vocab}"
            )
        except Exception as e:
            logging.warning(f"UE Token disabled: cannot resolve fid 97 from schema ({e})")
            ue_int_offset = -1
            ue_int_vocab = 0

    logging.info(f"Building PCVRHyFormer with cfg: {model_cfg}")

    # Collect all candidate kwargs.  Some older model.py versions do not accept
    # user_sparse_dense_pair_specs / ue_int_offset / ue_int_vocab, so we filter
    # by inspecting the actual __init__ signature at runtime.
    _model_kwargs: Dict[str, Any] = {
        ...
        "ue_int_offset": ue_int_offset,
        "ue_int_vocab": ue_int_vocab,
    }
    _model_kwargs.update(model_cfg)

    _sig = inspect.signature(PCVRHyFormer.__init__)
    _valid_params = set(_sig.parameters.keys())
    _filtered_kwargs = {k: v for k, v in _model_kwargs.items() if k in _valid_params}

    _skipped = set(_model_kwargs.keys()) - set(_filtered_kwargs.keys())
    if _skipped:
        logging.warning(
            f"Skipping parameters not accepted by this model.py: {sorted(_skipped)}"
        )

    model = PCVRHyFormer(**_filtered_kwargs).to(device)
