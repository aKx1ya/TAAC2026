...
def _batch_to_model_input(
    ...
    """Convert a batch dict to ModelInput, using the model's full seq
    domain list to guard against batches where a domain may be missing."""
    ...
    # Use model's complete domain list, not the batch's _seq_domains,
    # to avoid KeyError when a domain is missing from a particular batch.
    ...
    for domain in seq_domains:
        if domain in device_batch:
            ...
            # Domain missing from this batch — fill with padding zeros.
            B = device_batch["user_int_feats"].shape[0]
            L = 256  # safe default
            for d in model.seq_domains:
                if d in device_batch:
                    L = device_batch[d].shape[2]
                    break
            S = len(model._seq_vocab_sizes[domain])
            seq_data[domain] = torch.zeros(
                B, S, L, dtype=torch.long, device=device)
...
def main() -> None:
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            model_input = _batch_to_model_input(batch, device, model)
