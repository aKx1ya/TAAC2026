...
# user_dense layout: [61:256, 62:6, 63:19, 64:26, 65:111, 66:150, 87:320, 89:10, 90:10, 91:10]
# End offset = 256+6+19+26+111+150 = 568
# Fid 62-66 are processed by HierarchicalSparseDenseFusion inside NS tokenizer,
# so their dense columns must be EXCLUDED from the ordinary user_dense_proj path
# to avoid double-counting (same strategy as v9.0's UserSparseDensePairResidual).
USER_DENSE_PAIR_START = 256
USER_DENSE_PAIR_END = 568
USER_DENSE_MIN_HIERARCHICAL_DIM = 568
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Spec indices are resolved dynamically from fid→spec_index mapping so that
        # reordering/insertion of features in schema.json does not silently break the chain.
        # user_dense layout: [61:256, 62:6, 63:19, 64:26, 65:111, 66:150, 87:320, 89:10, 90:10, 91:10]
        _hierarchical_chain_info: Dict[int, Dict] = {}
        _hierarchical_fusion: Optional[HierarchicalSparseDenseFusion] = None
        if user_dense_dim >= USER_DENSE_MIN_HIERARCHICAL_DIM and user_int_fids:
            _fid_to_spec = {fid: i for i, fid in enumerate(user_int_fids)}
            _chain_fids = [62, 63, 64, 65, 66]
            _chain_spec_indices = [
                _fid_to_spec[fid] for fid in _chain_fids if fid in _fid_to_spec
            ]
            # Dense offsets are derived from the competition's fixed data schema
            # (user_dense columns are always ordered by fid).
            _chain_dense_offsets: List[Tuple[int, int]] = [
                (256, 6),    # fid 62  (offset after fid 61's 256 dims)
                (262, 19),   # fid 63
                (281, 26),   # fid 64
                (307, 111),  # fid 65
                (418, 150),  # fid 66
            ]
            if len(_chain_spec_indices) == len(_chain_fids) and all(
                _si < len(user_int_feature_specs) for _si in _chain_spec_indices
            ):
                ...
                logging.warning(
                    ...
                    f"some chain fids {_chain_fids} not found in user_int_fids "
                    f"(found {len(_chain_spec_indices)}/{len(_chain_fids)})"
                )
