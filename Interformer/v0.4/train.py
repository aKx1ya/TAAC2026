...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--no_time_order_split', dest='time_order_split',
                        action='store_false', default=True,
                        help='Disable time-ordered data split '
                             '(use legacy file-order split instead)')
    ...
    parser.add_argument('--seq_encoder_type', type=str, default='swiglu',
    ...
    parser.add_argument('--use_rope', action='store_true', default=True,
                        help='Enable RoPE positional encoding in sequence attention (default on)')
    parser.add_argument('--no_rope', dest='use_rope', action='store_false',
                        help='Disable RoPE positional encoding')
    parser.add_argument('--rope_base', type=float, default=10000.0,
                        help='RoPE base frequency (default 10000)')

    # Interaction Arch (paper-aligned).
    parser.add_argument('--interaction_backbone', type=str, default='dhen',
                        choices=['dhen', 'dot', 'dcn'],
                        help='Interaction backbone: dhen (DOT+DCN ensemble), dot, dcn')
    parser.add_argument('--interaction_layers', type=int, default=2,
                        help='Number of interaction layers inside Interaction Arch')
    parser.add_argument('--dcn_layers', type=int, default=2,
                        help='Number of DCN cross layers per DHEN/DCN block')
    parser.add_argument('--cross_low_rank', type=int, default=32,
                        help='Low-rank dimension for DCN cross layers')

    # Cross Arch (CLS / PMA / Recent tokens).
    parser.add_argument('--num_cls_tokens', type=int, default=4,
                        help='Number of CLS tokens for NS→Seq summarization')
    parser.add_argument('--num_pma_tokens', type=int, default=2,
                        help='Number of PMA (learnable query) tokens for Seq→NS')
    parser.add_argument('--num_recent_tokens', type=int, default=2,
                        help='Number of most-recent tokens for Seq→NS summary')

    # Feature engineering.
    parser.add_argument('--hash_bucket_size', type=int, default=100000,
                        help='Hash bucket size for ultra-high-cardinality features '
                             '(features with vocab > emb_skip_threshold use hash '
                             'embedding instead of zero; 0 = disable)')
    parser.add_argument('--item_vocab_size', type=int, default=0,
                        help='Shared item embedding vocab size. When > 0, the '
                             'top-level item_id and domain_c_seq_47 share one '
                             'embedding table (detected automatically by vocab match). '
                             'Set to 0 to disable sharing.')
    parser.add_argument('--item_id_seq_domain', type=str, default='seq_c',
                        help='Sequence domain that contains the item ID feature')

    # Dense feature preprocessing.
    parser.add_argument('--log_dense_fids', type=str, default='62,63,64,65,66',
                        help='Comma-separated user_dense feature IDs to apply '
                             'log(1+x) transform (statistical count/ratio features)')
    parser.add_argument('--sort_seq_domains', type=str, default='seq_d',
                        help='Comma-separated sequence domains whose sub-features '
                             'should be sorted within each time step for canonical '
                             'ordering (empty string = none)')
    ...
    parser.add_argument('--reinit_sparse_after_epoch', type=int, default=3,
    ...
    parser.add_argument('--item_ns_tokens', type=int, default=5,
...
def main() -> None:
    ...
    log_dense_fids: set[int] | None = None
    if args.log_dense_fids.strip():
        log_dense_fids = {int(x.strip()) for x in args.log_dense_fids.split(',') if x.strip()}
        logging.info(f"Log(1+x) transform on user_dense fids: {sorted(log_dense_fids)}")
    sort_seq_domains: set[str] | None = None
    if args.sort_seq_domains.strip():
        sort_seq_domains = {x.strip() for x in args.sort_seq_domains.split(',') if x.strip()}
        logging.info(f"Sort-seq-domains: {sorted(sort_seq_domains)}")
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        time_order_split=args.time_order_split,
    ...
    model_args = {
        ...
        "num_pma_tokens": args.num_pma_tokens,
        "num_recent_tokens": args.num_recent_tokens,
        "hash_bucket_size": args.hash_bucket_size,
        "item_vocab_size": args.item_vocab_size,
        "item_id_seq_domain": args.item_id_seq_domain,
    }
    ...
    logging.info(f"PCVRInterFormer model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, "
                 f"interaction_backbone={args.interaction_backbone}, use_rope={args.use_rope}")
    logging.info(f"Cross Arch: cls={args.num_cls_tokens}, pma={args.num_pma_tokens}, recent={args.num_recent_tokens}")
