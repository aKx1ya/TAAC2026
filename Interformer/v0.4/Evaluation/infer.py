...
from model import PCVRInterFormer, ModelInput
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        # ====== 必须新增的兼容代码段 开始 ======
        if key == 'num_blocks':
            # 优先读取 num_blocks，如果没有，则尝试读取旧版本的 num_hyformer_blocks，最后才是 fallback
            cfg[key] = train_config.get('num_blocks', train_config.get('num_hyformer_blocks', _FALLBACK_MODEL_CFG[key]))
            continue
        # ====== 必须新增的兼容代码段 结束 ======
...
def build_model(
    ...
    logging.info(f"Building PCVRInterFormer with cfg: {model_cfg}")
    model = PCVRInterFormer(
