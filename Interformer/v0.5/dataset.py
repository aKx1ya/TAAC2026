...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        rg_items: Optional[list[tuple[str, int, int, int]]] = None,
        log_dense_fids: Optional[set[int]] = None,
        sort_seq_domains: Optional[set[str]] = None,
    ) -> None:
        """
        Args:
            ...
            log_dense_fids: set of user_dense feature IDs whose values should
                be log(1+x) transformed.  Use for statistical count/ratio
                features (e.g. {62,63,64,65,66}) that have heavy-tailed
                distributions.
            sort_seq_domains: set of sequence domain names (e.g. {'seq_d'})
                whose sub-feature values should be sorted within each time
                step to ensure canonical ordering regardless of input column
                order.
        """
        super().__init__()
        ...
        self.log_dense_fids: set[int] = log_dense_fids or set()
        self.sort_seq_domains: set[str] = sort_seq_domains or set()
        # Pre-compute dense log-transform ranges: (start_offset, length) for
        # each contiguous block of fids that need log1p.
        self._log_dense_ranges: list[tuple[int, int]] = []  # filled after _load_schema
        ...
        if rg_items is not None:
            # Use the pre-computed (and possibly time-sorted) list.
            # Each item: (file_path, rg_idx, num_rows, sort_key)
            self._rg_list: list[tuple[str, int, int]] = [
                (f, i, n) for f, i, n, _k in rg_items
            ]
        else:
            self._rg_list = []
        ...
        # Pre-compute log(1+x) transform ranges for dense statistical features.
        if self.log_dense_fids:
            for fid, dim in self._user_dense_cols:
                if fid in self.log_dense_fids:
                    offset, length = self.user_dense_schema.get_offset_length(fid)
                    self._log_dense_ranges.append((offset, length))
            if self._log_dense_ranges:
                logging.info(
                    f"Log(1+x) transform on user_dense fids: "
                    f"{sorted(self.log_dense_fids)}"
                )
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # Log(1+x) transform on statistical dense features (e.g. counts/ratios).
        # Clamp to [-0.999, inf) first to prevent log1p(-1) → -inf or NaN
        # from rare anomalous values (e.g. raw value ≤ -1).
        for offset, length in self._log_dense_ranges:
            segment = user_dense[:, offset:offset + length]
            np.maximum(segment, -0.999, out=segment)
            np.log1p(segment, out=segment)
        ...
        for domain in self.seq_domains:
            ...
            # Canonical ordering for domains with inconsistent column layouts:
            # sort sub-feature values within each time step (along feature axis).
            if domain in self.sort_seq_domains:
                out.sort(axis=1)  # sort [feat, time] along feat dim per step
            out[out <= 0] = 0
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    When ``time_order_split=True`` (default), Row Groups across all Parquet
    files are sorted by their ``timestamp`` column statistics so that the
    validation split respects temporal order: the *earliest* data goes to
    training and the *latest* ``valid_ratio`` fraction goes to validation.

    This sorting uses only Parquet **metadata** (column statistics stored per
    Row Group) and does **not** read any row data, making it virtually free
    regardless of dataset size.

    When ``time_order_split=False``, the legacy behaviour is preserved: Row
    Groups are kept in file-name / in-file order and the validation split is
    simply the tail of that list.
    """
    random.seed(seed)
    ...
    # ── Locate the timestamp column index (same across all files) ──
    ts_col_idx: int = -1
    if time_order_split and pq_files:
        first_schema = pq.ParquetFile(pq_files[0]).schema_arrow
        try:
            ts_col_idx = first_schema.get_field_index('timestamp')
        except Exception:
            ts_col_idx = -1
        if ts_col_idx < 0:
            logging.warning(
                "time_order_split=True but 'timestamp' column not found in "
                "Parquet schema; falling back to file-order split."
            )
            time_order_split = False

    # ── Collect Row Group metadata ──
    # Each item: (file_path, rg_idx, num_rows, sort_key)
    # sort_key = timestamp_min when time_order_split else rg_idx
    rg_items: list[tuple[str, int, int, int]] = []
    for f in pq_files:
        pf = pq.ParquetFile(f)
        for i in range(pf.metadata.num_row_groups):
            num_rows = pf.metadata.row_group(i).num_rows
            sort_key = i  # fallback: in-file order
            if time_order_split:
                try:
                    col_meta = pf.metadata.row_group(i).column(ts_col_idx)
                    stats = col_meta.statistics
                    if stats is not None and hasattr(stats, 'min') and stats.min is not None:
                        sort_key = int(stats.min)
                except Exception:
                    pass
            rg_items.append((f, i, num_rows, sort_key))

    total_rgs = len(rg_items)

    if time_order_split:
        rg_items.sort(key=lambda x: x[3])
        logging.info(
            f"Time-order split: {total_rgs} Row Groups sorted by timestamp "
            f"(range: {rg_items[0][3]} → {rg_items[-1][3]})"
        )
    else:
        logging.info(
            f"File-order split: {total_rgs} Row Groups in glob/file order"
        )
    ...
    train_rows = sum(r[2] for r in rg_items[:n_train_rgs])
    valid_rows = sum(r[2] for r in rg_items[n_train_rgs:])
    ...
    train_dataset = PCVRParquetDataset(
        ...
        rg_items=rg_items[:n_train_rgs],
        log_dense_fids=log_dense_fids,
        sort_seq_domains=sort_seq_domains,
    )
