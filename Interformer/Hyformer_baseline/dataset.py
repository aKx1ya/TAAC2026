...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    validation split respects temporal order.
    ...
    if time_order_split and pq_files:
        ...
        try:
            ...
            pass
        if ts_col_idx < 0:
            logging.warning("time_order_split=True but 'timestamp' column not found in Parquet schema; falling back to file-order split.")
        ...
        for i in range(pf.metadata.num_row_groups):
            ...
            if time_order_split:
                try:
                    ...
                    if stats and hasattr(stats, 'min') and stats.min is not None:
                        sort_key = int(stats.min)
                except Exception:
                    pass
            # Make sure it matches what train_dataset uses: (filepath, rg_idx, num_rows, sort_key)
            # Actually, PCVRParquetDataset row_group_range expects rg_info entries without sort_key if we just pass range
            # Wait, `get_pcvr_data` parses `rg_info` directly. Let's fix this in next step.
            rg_info.append((f, i, num_rows, sort_key))

    total_rgs = len(rg_info)

    if time_order_split:
        rg_info.sort(key=lambda x: x[3])
        logging.info(f"Time-order split: {total_rgs} Row Groups sorted by timestamp (range: {rg_info[0][3]} → {rg_info[-1][3]})")
    else:
        logging.info(f"File-order split: {total_rgs} Row Groups in glob/file order")
    ...
    train_dataset = PCVRParquetDataset(
        ...
        row_group_range=rg_info[:n_train_rgs] if time_order_split else (0, n_train_rgs),
    ...
    valid_dataset = PCVRParquetDataset(
        ...
        shuffle=False,
        buffer_batches=0,
        row_group_range=rg_info[n_train_rgs:] if time_order_split else (n_train_rgs, total_rgs),
        clip_vocab=clip_vocab,
    )
