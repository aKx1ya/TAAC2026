...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_compile', action='store_true', default=False,
                        help='Enable torch.compile model compilation '
                             '(requires PyTorch >= 2.0, may slow the first epoch)')
...
def main() -> None:
    ...
    # ── Print schema.json content to log ──
    with open(schema_path, 'r', encoding='utf-8') as _f:
        _schema_content = json.load(_f)
    logging.info("===== schema.json content =====")
    logging.info(json.dumps(_schema_content, indent=2, ensure_ascii=False))
    logging.info("===== end of schema.json =====")
    # ────────────────────────────────────────
    ...
    # Optional: torch.compile (JIT compile model for GPU speedup)
    if args.use_compile and args.device.startswith('cuda'):
        try:
            model = torch.compile(model, mode='reduce-overhead')
            logging.info("torch.compile enabled (mode=reduce-overhead)")
        except Exception as e:
            logging.warning(f"torch.compile failed, falling back: {e}")
